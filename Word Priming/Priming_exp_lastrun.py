...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\Jash\\Downloads\\PsychoPy\\Word Priming\\Priming_exp_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    trials_1 = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('C:/Users/Jash/Downloads/PsychoPy/Word Priming/study_words.xlsx'), 
        seed=None, 
    )
    ...
    for thisTrial_1 in trials_1:
        ...
        trials_1.addData('slider.response', slider.getRating())
        trials_1.addData('slider.rt', slider.getRT())
    ...
    for thisTrial_2 in trial_2:
        ...
        # create an object to store info about Routine test_1
        test_1 = data.Routine(
            name='test_1',
            components=[text_2, textbox, text_3, mouse],
        )
        test_1.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        text_2.setText(test)
        textbox.reset()
        ...
        # store start times for test_1
        test_1.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        test_1.tStart = globalClock.getTime(format='float')
        test_1.status = STARTED
        thisExp.addData('test_1.started', test_1.tStart)
        test_1.maxDuration = None
        # keep track of which components have finished
        test_1Components = test_1.components
        for thisComponent in test_1.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        if isinstance(trial_2, data.TrialHandler2) and thisTrial_2.thisN != trial_2.thisTrial.thisN:
            continueRoutine = False
        test_1.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine and routineTimer.getTime() < 10.0:
            ...
            # check if all components have finished
            if not continueRoutine:  # a component has requested a forced-end of Routine
                test_1.forceEnded = routineForceEnded = True
                break
            continueRoutine = False  # will revert to True if at least one component still running
            for thisComponent in test_1.components:
                if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
        ...
        for thisComponent in test_1.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for test_1
        test_1.tStop = globalClock.getTime(format='float')
        test_1.tStopRefresh = tThisFlipGlobal
        thisExp.addData('test_1.stopped', test_1.tStop)
        trial_2.addData('textbox.text',textbox.text)
        # store data for trial_2 (TrialHandler)
        trial_2.addData('mouse.x', mouse.x)
        trial_2.addData('mouse.y', mouse.y)
        trial_2.addData('mouse.leftButton', mouse.leftButton)
        trial_2.addData('mouse.midButton', mouse.midButton)
        trial_2.addData('mouse.rightButton', mouse.rightButton)
        trial_2.addData('mouse.time', mouse.time)
        trial_2.addData('mouse.clicked_name', mouse.clicked_name)
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if test_1.maxDurationReached:
            routineTimer.addTime(-test_1.maxDuration)
        elif test_1.forceEnded:
            routineTimer.reset()
        else:
